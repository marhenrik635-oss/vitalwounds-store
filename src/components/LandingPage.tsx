...
export default function LandingPage({ navigateTo, isLoggedIn }: LPProps) {
  ...
  // Motion variants with safety reduced motion fallback
  ...
  return (
    <div className="min-h-[100dvh] bg-vw-bg text-vw-text antialiased overflow-x-hidden relative selection:bg-vw-accent/30">
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <a href="#" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: "smooth" }) }}
            ...
            <div className="w-8 h-8 bg-vw-text flex items-center justify-center rounded-lg group-hover:scale-[0.97] transition-transform">
              <span className="text-vw-bg font-display font-bold text-xs">VW</span>
            </div>
            <span className="font-display font-semibold tracking-tight text-vw-text">Vitalwounds</span>
          </a>
      ...
      <main id="content">
        ...
        <section className="relative pt-24 sm:pt-28 md:pt-32 pb-20 px-6 min-h-[90dvh] flex items-center">
          <div className="max-w-6xl mx-auto w-full">
            <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
              ...
              >
                <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold tracking-[-0.04em] leading-[1.05] text-balance">
                  {t("landing.hero.h1")}{" "}
        ...
        <section id="layanan" className="py-24 sm:py-32 px-6 border-t border-vw-border">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid lg:grid-cols-12 gap-8 items-start">
              {/* Left Column: Entertainment Essentials */}
              <div className="lg:col-span-7 space-y-6">
                <div className="font-display text-xs uppercase tracking-wider text-vw-accent font-bold mb-4">Entertainment Essentials</div>
                {APP_CARDS.slice(0, 3).map((app, i) => {
                  const Icon = app.icon
                  return (
                    <motion.div key={app.name} {...animProps(i * 100)}>
                      <div className="bg-vw-surface rounded-[1.5rem] p-6 sm:p-8 border border-vw-border hover:border-vw-accent/30 transition-all duration-300 flex flex-col sm:flex-row gap-6 items-start sm:items-center group">
                        <div className="w-12 h-12 rounded-xl bg-vw-accent/[0.08] flex items-center justify-center shrink-0 group-hover:bg-vw-accent/[0.12] transition-colors">
              ...
              {/* Right Column: Work & Creation */}
              <div className="lg:col-span-5 space-y-6">
                <div className="font-display text-xs uppercase tracking-wider text-vw-accent font-bold mb-4">Work & Creation</div>
                
                {/* Canva Pro - Asymmetric Featured Card */}
                <motion.div {...animProps(150)}>
                  <div className="bg-vw-accent/[0.03] rounded-[1.5rem] p-6 sm:p-8 border border-vw-accent/[0.12] hover:border-vw-accent/40 transition-all duration-300 flex flex-col group">
                    <div className="flex justify-between items-start mb-6">
                      <div className="w-12 h-12 rounded-xl bg-vw-accent/[0.1] flex items-center justify-center">
                        <Palette size={20} className="text-vw-accent" />
                      </div>
                      <span className="text-[10px] uppercase font-bold tracking-widest text-vw-accent bg-vw-accent/[0.1] px-3 py-1 rounded-full">POPULAR</span>
                    </div>
                    <h3 className="font-display text-xl font-semibold mb-2 text-vw-text">Canva Pro</h3>
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-vw-accent/[0.12]">
                      <div>
                        <span className="text-[10px] uppercase text-vw-text-muted block mb-0.5">Harga Spesial</span>
                        <span className="font-display text-xl font-bold text-vw-text">Rp 25.000</span>
                      </div>
                      <button onClick={() => navigateTo(isLoggedIn ? "dashboard-panel" : "auth", "dashboard")}
                  ...
                  {APP_CARDS.slice(4, 6).map((app, i) => {
                    ...
                    return (
                      <motion.div key={app.name} {...animProps(200 + i * 100)} className="h-full">
                        <div className="bg-vw-surface rounded-[1.5rem] p-6 border border-vw-border hover:border-vw-accent/30 transition-all duration-300 flex flex-col h-full justify-between group">
                          <div>
                            <div className="w-10 h-10 rounded-xl bg-vw-accent/[0.06] flex items-center justify-center mb-4 group-hover:bg-vw-accent/[0.1] transition-colors">
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-vw-border/80 mt-auto">
                            <span className="font-display text-base font-bold text-vw-text">{app.price}</span>
                            <button onClick={() => navigateTo(isLoggedIn ? "dashboard-panel" : "auth", "dashboard")}
              ...
              {[
                ...
                <motion.div key={item.label} {...animProps(i * 120)}>
                  <div className="text-center sm:text-left relative">
                    <div className="w-14 h-14 rounded-[1.25rem] bg-vw-bg flex items-center justify-center mx-auto sm:mx-0 mb-6 border border-vw-border relative z-10">
        ...
        {/* ====== WHY US - Left heading + right list ====== */}
              ...
              <div className="lg:col-span-7 space-y-6">
                {[
                  ...
                  <motion.div key={w.title} {...animProps(i * 80)}>
                    <div className={`p-6 rounded-[1.25rem] transition-colors border ${w.accent ? "bg-vw-accent/[0.04] border-vw-accent/[0.15]" : "bg-vw-surface border-vw-border"}`}>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="border-t border-vw-border pt-16">
              <div className="grid lg:grid-cols-3 gap-8 items-start">
                <motion.div {...animProps()}>
                  <h3 className="font-display text-lg font-semibold mb-2 text-vw-text">{t("landing.payment.title")}</h3>
                ...
                <div className="lg:col-span-2 flex flex-wrap gap-4">
                  {[
                    ...
                    <motion.div key={pm.name} {...animProps(i * 50)}>
                      <div className="flex items-center gap-4 bg-vw-surface border border-vw-border rounded-[1rem] px-5 py-3">
      ...
      <footer className="border-t border-vw-border py-16 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-16">
            ...
            <div>
              <h3 className="font-display text-xs font-bold text-vw-text-muted uppercase tracking-wider mb-4">{t("landing.footer.layanan")}</h3>
            ...
            <div>
              <h3 className="font-display text-xs font-bold text-vw-text-muted uppercase tracking-wider mb-4">{t("landing.footer.kontak")}</h3>
              <ul className="space-y-3">
